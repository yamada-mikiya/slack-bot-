...
# 🔹 特定の団体名を含むユーザーだけを対象とする
def user_in_group(real_name):
    return any(group in real_name for group in GROUP_NAMES)

# 🔹 ユーザー名 → ユーザーID の変換（フィルタ済み）
target_user_ids = {
    user['id']: user.get('real_name')
    for user in USERS_LIST
    if user.get('real_name') and user_in_group(user['real_name'])
}
